...
def run_comprehensive_audit():
    ...
    record("Auth Login (Valid Credentials & Session Cookie)", login_success, f"User: {res.json().get('user', {}).get('email')}")

    # 1.3 Get Me profile
    res = client.get("/api/v1/frontend/auth/me")
    me_data = res.json()
    record("Auth /me Endpoint & Active Workspace Resolution", res.status_code == 200 and "active_workspace" in me_data, f"Workspace: {me_data.get('active_workspace', {}).get('name')}")
    ...
    ov = res.json()
    record("Overview Real Metrics Aggregator", res.status_code == 200 and "total_posts" in ov and "total_reach" in ov, f"Total Posts: {ov.get('total_posts')}, Scheduled: {ov.get('scheduled_posts')}")
    ...
    record("Social Account Manual Create/Connect", res.status_code == 200 and created_acc.get("platform") == "instagram", f"Account ID: {created_acc.get('id')}")
    ...
    record("Social Accounts List Query", res.status_code == 200 and len(acc_list) >= 2, f"Total Accounts: {len(acc_list)}")

    # 3.3 OAuth Init Platform Check
    res = client.get("/api/v1/frontend/dashboard/accounts/oauth-init?platform=instagram")
    record("OAuth Discovery & Graceful Developer Check", res.status_code == 200 and "configured" in res.json(), f"Configured: {res.json().get('configured')}")
    ...
    record("Post Creation & Multi-platform Scheduling", res.status_code == 200 and post_id is not None, f"Post ID: {post_id}")

    # 4.2 List Posts
    res = client.get("/api/v1/frontend/dashboard/posts")
    posts_list = res.json().get("posts", [])
    record("Post Listing & Media/Target Relations", res.status_code == 200 and len(posts_list) >= 1, f"Found {len(posts_list)} posts")

    # 4.3 Approve Post
    if post_id:
        res = client.post(f"/api/v1/frontend/dashboard/posts/{post_id}/approve")
        record("Post Approval Workflow Action (Approve)", res.status_code == 200 and res.json().get("success") is True, f"Approval Status: {res.json().get('approval_status')}")

    # 4.4 Reject Post
    if post_id:
        res = client.post(f"/api/v1/frontend/dashboard/posts/{post_id}/reject")
        record("Post Rejection Workflow Action (Reject)", res.status_code == 200 and res.json().get("success") is True, f"Approval Status: {res.json().get('approval_status')}")

    # -------------------------------------------------------------
    # 5. CALENDAR & TIME SLOTS
    # -------------------------------------------------------------
    print("\n--- [5/11] AUDITING CALENDAR & SCHEDULE DISTRIBUTION ---")
    res = client.get("/api/v1/frontend/dashboard/calendar?start_date=2026-08-01T00:00:00Z&end_date=2026-08-31T23:59:59Z")
    cal_data = res.json()
    record("Calendar Events & Posting Slots Retrieval", res.status_code == 200 and "events" in cal_data, f"Events Count: {len(cal_data.get('events', []))}")
    ...
    record("Kanban Board 4-Column Aggregation", res.status_code == 200 and len(kb_data) == 4, f"Columns: {[c['id'] for c in kb_data]}")
    ...
    if idea_id:
        ...
        record("Kanban Idea Status Transition", res.status_code == 200 and res.json().get("status") == "in_progress", f"New Status: in_progress")

    # 6.4 Delete Idea
    if idea_id:
        res = client.delete(f"/api/v1/frontend/dashboard/kanban/{idea_id}")
        record("Kanban Idea Delete Action", res.status_code == 200 and res.json().get("success") is True, "Deleted successfully")
    ...
    record("Inbox Message List & NLP Sentiment Classification", res.status_code == 200 and sentiment_correct, f"Sender: {target_msg.get('sender_name') if target_msg else 'None'}, Sentiment: {target_msg.get('sentiment') if target_msg else 'None'}")
    ...
    record("Inbox Reply Dispatch & Status Resolve", res.status_code == 200 and res.json().get("status") == "resolved", f"Reply ID: {res.json().get('reply_id')}")
    ...
    an_data = res.json()
    record("Analytics KPIs, Daily Trends, Channel Breakdown", res.status_code == 200 and "kpis" in an_data and len(an_data.get("trends", [])) > 0, f"Followers KPI: {an_data.get('kpis', {}).get('total_followers')}, Trends points: {len(an_data.get('trends', []))}")
    ...
    record("Media File Upload & Metadata Extraction", res.status_code == 200 and media_id is not None, f"Asset ID: {media_id}, Title: {media_asset.get('title')}")

    # 9.2 List Media Assets
    res = client.get("/api/v1/frontend/dashboard/media")
    media_list = res.json().get("assets", [])
    record("Media Assets Query & Thumbnail Resolution", res.status_code == 200 and len(media_list) >= 1, f"Total Assets: {len(media_list)}")

    # 9.3 Delete Media Asset
    if media_id:
        res = client.delete(f"/api/v1/frontend/dashboard/media/{media_id}")
        record("Media Asset Deletion & Disk Cleanup", res.status_code == 200 and res.json().get("success") is True, "Asset deleted")
    ...
    record("Team Member Invite & Account Provisioning", res.status_code == 200 and member_id is not None, f"Member: {invited_member.get('name')}, Role: {invited_member.get('role')}")

    # 10.2 List Members
    res = client.get("/api/v1/frontend/dashboard/members")
    members_data = res.json().get("members", [])
    record("Team Members Listing Query", res.status_code == 200 and len(members_data) >= 2, f"Total Members: {len(members_data)}")
    ...
    if member_id:
        ...
        record("Member Role Elevation (Editor -> Manager)", res.status_code == 200 and res.json().get("role") == "manager", "Elevated to Manager")

    # 10.4 Remove Member
    if member_id:
        res = client.delete(f"/api/v1/frontend/dashboard/members/{member_id}")
        record("Member Removal from Workspace", res.status_code == 200 and res.json().get("success") is True, "Removed successfully")
    ...
    sett = res.json()
    record("Workspace Settings Query", res.status_code == 200 and "organization_name" in sett, f"Org: {sett.get('organization_name')}, Mode: {sett.get('approval_workflow_mode')}")
    ...
    record("Workspace Settings Mutation", res.status_code == 200 and res.json().get("success") is True, "Settings updated")
