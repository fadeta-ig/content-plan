...
def _summarize_platform_statuses(statuses: list[str]) -> str:
    if not statuses:
        return "draft"
    for status in ("failed", "pending_review", "pending_client", "publishing", "scheduled"):
...
def dashboard_overview(request: HttpRequest):
    ...
    pending_approvals = (
        posts_qs.filter(platform_posts__status__in=["pending_review", "pending_client"]).distinct().count()
    )
    ...
    recent = posts_qs.prefetch_related("platform_posts__social_account", "media_attachments__media_asset").order_by(
        "-created_at"
    )[:6]
    ...
    for p in recent:
        ...
        primary_status = _summarize_platform_statuses(statuses)
        thumbnail = ""
        first_media = p.media_attachments.first()
        if first_media and first_media.media_asset and first_media.media_asset.file:
            thumbnail = (
                first_media.media_asset.thumbnail.url
                if first_media.media_asset.thumbnail
                else first_media.media_asset.file.url
            )
