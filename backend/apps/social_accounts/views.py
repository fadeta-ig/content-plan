...
def connect_platform(request, workspace_id):
    """GET: show platform grid (or initiate OAuth if ?platform= provided). POST: initiate OAuth flow."""
    configured_platforms = _get_configured_platforms(request.org.id)
    visible_platform_choices = _get_visible_platform_choices()

    # Determine platform from POST body or GET query param
    platform = (request.POST.get("platform") or request.GET.get("platform") or "").strip()
    ...
    if platform not in configured_platforms:
        messages.error(
            request,
            f"Platform credentials for {platform} are not configured. Please configure them in your settings or .env file.",
        )
        return redirect("social_accounts:connect", workspace_id=workspace_id)
...
@login_required
def connect_platform_redirect(request):
    """Fallback handler for /social-accounts/connect/ without workspace_id in URL."""
    from apps.members.models import WorkspaceMembership

    workspace = getattr(request, "workspace", None)
    if not workspace:
        membership = WorkspaceMembership.objects.filter(user=request.user).select_related("workspace").first()
        if membership:
            workspace = membership.workspace

    if not workspace:
        messages.error(request, "Tidak ada workspace aktif yang ditemukan.")
        return redirect("dashboard")

    platform = request.GET.get("platform", "").strip()
    if platform:
        return redirect(f"/social-accounts/{workspace.id}/connect/?platform={platform}")
